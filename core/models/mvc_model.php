...
class MvcModel {
	...
	protected function init_schema() {
		...
		foreach ($results as $result) {
			...
			$this->schema_add_column(
				$field,
				($result->Key ? $result->Key : null),
				$type,
				$length,
				$result->Null == 'YES' ? true : false,
				$result->Default
			);
		}
	}

	protected function schema_clear() {
		$this->schema = null;
		$this->db_adapter->schema = null;
	}

	protected function schema_add_column($fieldName, $key, $type, $length = null, $nullable = false, $default = null) {
		if ($this->schema == null) {
			$this->schema = array();
		}
		$column = array();
		...
		$column['null'] = $nullable;
		$column['default'] = $default;
		$this->schema[$fieldName] = $column;
		$this->db_adapter->schema = $this->schema;
	}
